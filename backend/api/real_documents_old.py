...
async def download_real_documents(tender_id: str, db = Depends(get_db)):
    ...
    try:
        ...
        result = await download_real_pdfs_for_tender(tender_id)
        ...
        if result and result["downloaded_files"]:
            return {
                ...
                "download_folder": result["folder_path"]
            }
        else:
            return {
...
async def list_real_documents(tender_id: str, db: Session = Depends(get_db)):
    """
    ...
    for folder in possible_folders:
        if folder.exists():
            ...
            # List PDF files
            for pdf_file in folder.glob("*.pdf") | folder.glob("*.PDF"):
...
async def serve_real_document(tender_id: str, filename: str, db: Session = Depends(get_db)):
    """
